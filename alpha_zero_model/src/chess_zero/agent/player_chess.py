...
from alpha_zero_model.src.chess_zero.config import Config
from alpha_zero_model.src.chess_zero.env.chess_env import ChessEnv, Winner
...
class ChessPlayer:
    ...
    def action(self, env, can_stop = True) -> str:
        ...
        top_3_actions_indices = np.argsort(policy)[-3:][::-1]  # 확률이 높은 순으로 3개의 인덱스
        top_3_actions = [(self.config.labels[i], policy[i]) for i in top_3_actions_indices]

    # 상위 3개의 수와 해당 확률을 출력
        print("Top 3 actions:")
        for action, prob in top_3_actions:
    ...
    def search_my_move(self, env: ChessEnv, is_root_node=False) -> float:
        ...
        This method searches for possible         print("mymove",self.moves)
            print("root_value",root_value)
            print("policy",policy)moves, adds them to a search tree, and eventually returns the
