...
class EvaluateWorker:
    ...
    def move_model(self, model_dir):
        ...
        # model_dir을 Path 객체로 변환
        model_dir_path = Path(model_dir)
        # Path 객체의 .name 속성을 사용하여 디렉토리 이름 접근
        new_dir = os.path.join(rc.next_generation_model_dir, "copies", model_dir_path.name)
        # os.rename을 사용할 때 Path 객체 대신 문자열 경로 사용
        os.rename(model_dir_path, Path(new_dir))
