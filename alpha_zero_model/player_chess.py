...
class ChessPlayer:
    ...
    def action(self, env, can_stop=True) -> str:
        ...
        if can_stop and self.play_config.resign_threshold is not None \
                and root_value <= self.play_config.resign_threshold \
                and env.num_halfmoves > self.play_config.min_resign_turn:
            logging.info("No action taken (resign).")
            return None
        else:
            action_str = self.config.labels[my_action]
            self.moves.append([env.observation, list(policy)])
